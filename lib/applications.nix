...
# external service's address and never a value for one; `image` is a repository with no tag,
# because which build runs is a deployment's decision and a digest is one deployment's proof of
...
# else's numbers here would dress a guess as knowledge. Replica counts, node selectors, identities
# and addresses: fleet facts, every one of them, and the grammar refuses several of them outright.
{}:
{
  applications = {
    # ─────────────────────────────────────────────────────────────────────────────────────────
    ...
    nextcloud = {
      ...
      requires = {
        ...
        # are the official image's own contract for a PostgreSQL server; the PASSWORD is
        # deliberately not among them, because a credential arrives from a Secret and nothing in
        # this repository may carry one.
    ...
    rclone = {
      ...
      note = ''
        ...
        IT AUTHENTICATES, AND WEAKLY. The API and the GUI take HTTP basic auth from two variables,
        and that is the whole of it -- no accounts, no sessions, no second factor. Anything that
        reaches the port and knows the pair can read and rewrite every remote definition, which
        for an app whose entire job is holding credentials to other people's storage means the
        exposure class is not a detail. The credential itself arrives from a Secret; nothing here
        can carry one.
